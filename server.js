...
/* ============================= SOCKET.IO ============================= */
const QUESTION_TIME_LIMIT_MS = 20000; // 20 secondes par question
const closeTimers = {}; // code -> Timeout (transient, not persisted)
...
io.on('connection', (socket) => {
  ...
  socket.on('state:update', ({ code, state }) => {
    ...
    // Cancel any pending auto-close timer for this session; a new one is scheduled below if needed.
    if (closeTimers[code]) { clearTimeout(closeTimers[code]); delete closeTimers[code]; }

    if (state.phase === 'question') {
      const qIdx = state.currentQuestionIndex;
      closeTimers[code] = setTimeout(() => {
        delete closeTimers[code];
        const cur = db.sessions[code];
        if (cur && cur.state.phase === 'question' && cur.state.currentQuestionIndex === qIdx) {
          cur.state = Object.assign({}, cur.state, { phase: 'closed' });
          scheduleSave();
          io.to(code).emit('state:update', cur.state);
        }
